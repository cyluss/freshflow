...
// 이슈 #38: "가능"은 오늘 미리보기(preview)가 이미 그날의 유효 기준에 닿아 있다는
// 뜻이다. 유효 기준은 #37과 같은 원칙으로 quota와 그날 예상 주문(est) 중 작은 쪽이다
// - 예상 주문 자체가 quota보다 적어도 미리보기가 그 예상 주문만큼 채워지고 있으면
// "이행 가능"이다(예전에는 quota 하나만 보고 예상 주문이 적은 날마다 "이행 어려움"을
// 냈다 - #37이 이슈 개폐 조건에서 고친 것과 같은 문제가 이 판정에도 남아 있었다).
// 확정이 아니라 오늘 조건 판정이다.
FF.issueFeasible=function(preview,est,quota){
 var left=FF.C.days-FF.dayOf();
 if(left<FF.C.rel.up)return "low";
 var effQuota=Math.min(quota,est);
 return preview>=effQuota-FF.C.ui.zero?"ok":"hard";
}
...
FF.issuePlan=function(){
 ...
 for(var i=0;i<issues.length;i++){
  ...
  openList.push({i:i,since:issues[i].since,resolution:issues[i].resolution,
   days:FF.dayOf()-issues[i].since,rel:rel[i],
   feasible:FF.issueFeasible(P.preview[i],P.est[i],P.rows[i].quota),
