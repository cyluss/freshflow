...
// 이슈 #38: issueFeasible도 #37과 같은 유효 기준(min(quota,예상 주문))을 써야 한다.
// #37은 이슈가 여닫히는 조건만 고쳤는데, 이미 열린 이슈 카드에 뜨는 "이행 가능/어려움"
// 판정은 여전히 raw quota만 보고 있어서, 예상 주문 자체가 quota보다 적은 날에는
// 오늘 미리보기를 그 예상 주문만큼(=100%) 채우고 있어도 "이행 어려움"이 떴다
// (#29 실측 버그: 프랜차이즈 예상 판매 5t · 약속 이행 기준 10t · 현재 이행 어려움,
// 그런데 배정 5t/주문 5t로 이미 100% 채워진 상태였다).
{
  FF.reset(1);
  const days = FF.C.days;
  // 예상 주문(est) 자체가 quota보다 적어도, 오늘 미리보기(preview)가 그 예상 주문만큼
  // 채워지고 있으면 이행 가능이어야 한다 - #29가 실측한 정확한 모양이다.
  t('#29 재현: 주문<quota인데 100% 채우면 이행 가능', FF.issueFeasible(5, 5, 10) === 'ok');
  // 예상 주문(est)이 quota 이상인데 오늘 미리보기가 그에 못 미치면(경쟁 등으로 밀림)
  // 여전히 이행 어려움이어야 한다 - #37 이전처럼 이 경우까지 "가능"으로 바뀌면 안 된다.
  t('주문>=quota인데 못 채우면 이행 어려움', FF.issueFeasible(4, 10, 10) === 'hard');
  // 주문 자체가 quota보다 적은데 그마저도 다 못 채우면(경쟁으로 밀림) 이행 어려움이다.
  t('주문<quota인데도 못 채우면 이행 어려움', FF.issueFeasible(3, 5, 10) === 'hard');
  // 게임 종료가 임박하면(회복 주기보다 남은 날이 적으면) 무조건 실익 낮음이다.
  FF.reset(1);
  for (let d = 1; d < days; d++) FF.stepDay(FF.Cmd.wait());
  t('종료 임박이면 실익 낮음', FF.issueFeasible(5, 5, 10) === 'low');
}
